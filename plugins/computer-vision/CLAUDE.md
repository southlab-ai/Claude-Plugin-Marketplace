# Computer Vision Plugin for Claude Code — Developer Guide

## Overview
MCP plugin giving Claude Code desktop vision and input control on Windows. 24 tools.
Usage instructions are in `src/server.py` → `FastMCP(instructions=...)` and injected
into every conversation's system prompt automatically. This file is for **developers
editing the plugin code**, not for end users.
...
- **Server instructions**: Usage guide is in `FastMCP(instructions=...)` in `src/server.py`
